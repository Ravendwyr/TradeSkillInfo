...
TradeskillInfoUI.version = "1.0." .. string.sub("$Revision: 58106 $", 12, -3);
TradeskillInfoUI.date = string.sub("$Date: 2008-01-09 03:06:17 -0500 (Wed, 09 Jan 2008) $", 8, 17);
...
TradeskillInfoUI.options.buttons = {
	["TradeskillInfoOpposingButton"] = { text = "对方阵营", var = "ShowOpposing", tooltip="包含对方阵营的配方" },
	["TradeskillInfoNameButton"] = { text = "名称", var = "SearchName", tooltip="以名称来查询"},
	["TradeskillInfoReagentButton"] = { text = "材料", var = "SearchReagent", tooltip="以材料来查询"},
};
...
-- Draw frame with tradeskill info
	...
	for i=1, TradeskillInfoUI.cons.skillsDisplayed, 1 do
		...
		if ( skillIndex <= numTradeSkills ) then	
			local skillName, skillType, isExpanded, skillLevel = self:GetTradeSkillInfo(skillIndex);
			...
			if ( skillType == "header" ) then
				...
				skillButton:SetText(" "..skillName.." ["..skillLevel.."]");
...
function TradeskillInfoUI:ShowReagentTooltip(this)
	...
	if this.known then
		...
		GameTooltip:AddLine("本地缓存中没有该物品.");
		GameTooltip:AddLine("左键点击尝试更新该物品.");
		GameTooltip:AddLine("注意! 这可能导致你与服务器断开连接.");
		GameTooltip:Show();
	end
end
...
function TradeskillInfoUI:AvailabilityDropDown_OnLoad()
	...
	UIDropDownMenu_SetText("过滤方式", TradeskillInfoAvailabilityDropDown);
end
...
function TradeskillInfoUI:TradeskillsDropDown_OnLoad()
	UIDropDownMenu_SetWidth(120);
	UIDropDownMenu_SetText("技能", TradeskillInfoTradeskillsDropDown);
end
...
function TradeskillInfoUI:GetTradeSkillInfo(index)
	...
	ItemAllInfo = TradeskillInfo:GetCombine(self.vars.searchResult[index])
	return skillName, "combine", true, ItemAllInfo.level;
end
...
function TradeskillInfoUI:GetTradeSkillBy(index)
	...
	local text
	if knownBy then
		text = "|Cffffffff已经学会: "..knownBy.."|r";
	end
	if learnableBy then
		if not text then
			text = "|Cff33ff33现在可学: "..learnableBy.."|r";
		else
			text = text.."\n|cff33ff33现在可学: "..learnableBy.."|r";
		end
	end
	if availableTo then
		if not text then
			text = "|Cffff7733将来可学: "..availableTo.."|r";
		else
			text = text.."\n|Cffff7733将来可学: "..availableTo.."|r"
		end
	end
	return text;
end
