...
TradeskillInfoUI.version = "1.0." .. string.sub("$Revision: 72793 $", 12, -3);
TradeskillInfoUI.date = string.sub("$Date: 2008-05-05 14:14:14 -0400 (Mon, 05 May 2008) $", 8, 17);
TradeskillInfoUI:RegisterDB("TradeskillInfoUIDB")
TradeskillInfoUI:RegisterDefaults('profile', {
	tradeskills = {['A']=true,['B']=true,['D']=true,['E']=true,['J']=true,['L']=true,['T']=true,['W']=true,['X']=true,['Z']=true,['Y']=true,},
	expanded = {['A']=true,['B']=true,['D']=true,['E']=true,['J']=true,['L']=true,['T']=true,['W']=true,['X']=true,['Z']=true,['Y']=true,},
...
TradeskillInfoUI.options.buttons = {
	["TradeskillInfoOpposingButton"] = { text = "Opuesta", var = "ShowOpposing", tooltip="Incluye recipientes de la facción opuesta" },
	["TradeskillInfoNameButton"] = { text = "Nombre", var = "SearchName", tooltip="Buscar por nombre"},
	["TradeskillInfoReagentButton"] = { text = "Reactivo", var = "SearchReagent", tooltip="Buscar por reactivos"},
};
...
TradeskillInfoUI.vars.availability = {
	"Aprendido por el Jugador",
	"El Jugador puede aprenderlo",
	"El Jugador podrá aprenderlo",
	"Aprendido",
	"Puede aprenderlo",
	"Podrá aprenderlo",
	"No disponible",
...
function TradeskillInfoUI:ShowReagentTooltip(this)
	...
	if this.known then
		...
		GameTooltip:AddLine("El elemento no esta en el cache local.");
		GameTooltip:AddLine("Click para intentar actualizar el cache local.");
		GameTooltip:AddLine("¡Aviso! Puedes ser desconectado.");
		GameTooltip:Show();
	end
end
...
function TradeskillInfoUI:AvailabilityDropDown_OnLoad()
	...
	UIDropDownMenu_SetText("Disponibilidad", TradeskillInfoAvailabilityDropDown);
end
...
function TradeskillInfoUI:TradeskillsDropDown_OnLoad()
	UIDropDownMenu_SetWidth(120);
	UIDropDownMenu_SetText("Habilidades de Comercio", TradeskillInfoTradeskillsDropDown);
end
...
function TradeskillInfoUI:GetTradeSkillBy(index)
	...
	local text
	if knownBy then
		text = "|CffffffffConocido Por: "..knownBy.."|r";
	end
	if learnableBy then
		if not text then
			text = "|Cff33ff33Puede Aprenderlo: "..learnableBy.."|r";
		else
			text = text.."\n|cff33ff33Puede Aprenderlo: "..learnableBy.."|r";
		end
	end
	if availableTo then
		if not text then
			text = "|Cffff7733Podrá Aprenderlo: "..availableTo.."|r";
		else
			text = text.."\n|Cffff7733Podrá Aprenderlo: "..availableTo.."|r"
		end
	end
	return text;
end
...
function TradeskillInfoUI:PasteRecipie(id)
	...
	if enchantId then
		...
		skillLink = "|cffffd000|HEncantamiento:" .. enchantId .. "|h[" .. enchantName .. "]|h|r";
		if skillLink then
