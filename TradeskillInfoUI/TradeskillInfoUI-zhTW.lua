...
TradeskillInfoUI.version = "1.0." .. string.sub("$Revision: 70018 $", 12, -3);
TradeskillInfoUI.date = string.sub("$Date: 2008-04-16 02:43:10 -0400 (Wed, 16 Apr 2008) $", 8, 17);
...
TradeskillInfoUI.options.buttons = {
	["TradeskillInfoOpposingButton"] = { text = "對方陣營", var = "ShowOpposing", tooltip="包含對方陣營的配方" },
	["TradeskillInfoNameButton"] = { text = "名稱", var = "SearchName", tooltip="以名稱來查詢"},
	["TradeskillInfoReagentButton"] = { text = "材料", var = "SearchReagent", tooltip="以材料來查詢"},
};
...
function TradeskillInfoUI:AvailabilityDropDown_OnLoad()
	...
	UIDropDownMenu_SetText("過濾模式", TradeskillInfoAvailabilityDropDown);
end
...
function TradeskillInfoUI:GetTradeSkillBy(index)
	...
	local text
	if knownBy then
		text = "|Cffffffff已經學會: "..knownBy.."|r";
	end
	if learnableBy then
		if not text then
			text = "|Cff33ff33現在可學: "..learnableBy.."|r";
		else
			text = text.."\n|cff33ff33現在可學: "..learnableBy.."|r";
		end
	end
	if availableTo then
		if not text then
			text = "|Cffff7733將來可學: "..availableTo.."|r";
		else
			text = text.."\n|Cffff7733將來可學: "..availableTo.."|r"
		end
	end
	return text;
end
