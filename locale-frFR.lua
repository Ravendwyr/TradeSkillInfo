local L = LibStub("AceLocale-3.0"):NewLocale("TradeskillInfo", "frFR", false)
if not L then return end
L["Alchemy"] 													= "Alchimie"
L["Blacksmithing"] 												= "Forge"
L["Enchanting"] 												= "Enchantement"
L["Engineering"] 												= "Ingénierie"
L["Jewelcrafting"] 												= "Joaillerie"
L["Leatherworking"] 											= "Travail du cuir"
L["Tailoring"] 													= "Couture"
L["Cooking"] 													= "Cuisine"
L["First Aid"] 													= "Secourisme"
L["Poisons"] 													= "Poisons"
L["Smelting"] 													= "Fondre"
L["Armorsmith"] 												= "Fabricant d'armures"
L["Weaponsmith"] 												= "Fabricant d'armes"
L["Master Swordsmith"] 											= "Fabricant d'épées"
L["Master Hammersmith"] 										= "Fabricant de marteaux"
L["Master Axesmith"] 											= "Fabricant de haches"
L["Dragonscale Leatherworking"] 								= "Travail du cuir d'écailles de dragon"
L["Elemental Leatherworking"] 									= "Travail du cuir élémentaire"
L["Tribal Leatherworking"] 										= "Travail du cuir tribal"
L["Gnomish Engineer"] 											= "Ingénieur gnome"
L["Goblin Engineer"] 											= "Ingénieur gobelin"
L["Spellfire Tailoring"] 										= "Couture du feu-sorcier"
L["Shadoweave Tailoring"] 										= "Couture de tisse-ombre"
L["Mooncloth Tailoring"] 										= "Couture d'étoffe lunaire"
L["Herbalism"] 													= "Herboristerie"
L["Disenchant"] 												= "Désenchanter"
L["Prospecting"] 												= "Prospection"
L["Inscription"] 												= "Calligraphie"
L["Milling"] 													= "Mouture"
L["Mining"] 													= "Minage"
L["Skinning"] 													= "Dépecage"
L["Fishing"] 													= "Pêche"
L["Pick Pocket"] 												= "Vol à la tire"
-- Other Sources
L["Vendor"] 													= "Vendeur"
L["Alliance Vendor"] 											= "Vendeur de l'Alliance"
L["Horde Vendor"] 												= "Vendeur de la Horde"
L["Dropped"] 													= "Butin"
L["Crafted"] 													= "Créé"
L["Gathered"] 													= "Récolté"
L["Quest"] 														= "Quête"
L["Alliance Quest"] 											= "Quête de l'Alliance"
L["Horde Quest"] 												= "Quête de la Horde"
L["Trainer"] 													= "Maître"
L["Specialist Trainer"] 										= "Maître des Spécialisations"
L["Not Currently Obtainable"] 									= "Pas encore disponible"
L["Unknown"] 													= "Inconnu"
-- Other strings
L["TradeskillInfo"] 											= "TradeskillInfo"
L["Complete Tradeskill Info"] 									= "Information complète des métiers"
L["Source"] 													= "Source"
L["Vendor sell price"] 											= "Prix d'achat au vendeur"
L["ItemID"] 													= "ID de l'objet"
L["Stack size"] 												= "Taille de la pile"
L["Combine profit"] 											= "Profit combiné"
L["Show cost of combine and profit"] 							= "Afficher le coût des combinés et du profit"
L["Combine level"] 												= "Niveau combiné"
L["Show skill level of the combines"] 							= "Afficher le niveau des combinés"
L["Show the source of the item"] 								= "Afficher les sources de l'objet"
L["Used in"] 													= "Utilisé en"
L["Show what tradeskill an item is used in in it's tooltip"] 	= "Afficher par quel métier l'objet est utilisé dans le tooltip"
L["Usable by"] 													= "Utilisable par"
L["Show who can use an item in it's tooltip"] 					= "Afficher qui peut utiliser l'objet dans le tooltip"
L["Known by"] 													= "Connu de"
L["Show who knows a recipe in it's tooltip"] 					= "Afficher qui connaît une recette dans le tooltip"
L["Learnable by"] 												= "Peut être appris par"
L["Show who can learn a recipe in it's tooltip"] 				= "Afficher qui peut apprendre une recette dans le tooltip"
L["Will be able to learn"] 										= "Pourra être appris par"
L["Show who will be able to learn a recipe in it's tooltip"] 	= "Afficher qui pourra apprendre une rcette dans le tooltip"
L["Reagents"] 													= "Composants"
L["Show what reagents a recipe takes at the trainer"] 			= "Afficher les composants requis pour une recette au maître"
L["Config"] 													= "Configuration"
L["Show config screen"] 										= "Afficher l'écran de configuration"
L["Menu"] 														= "Menu"
L["Show TradeskillInfo UI"] 									= "Montrer TradeskillInfo UI"
L["Banked"] 													= "En banque"
L["Alts have"] 													= "Les autres en ont"
L["Found unknown skill"] 										= "Découvert une compétence inconnue"
L["Found unknown source"] 										= "Découvert une source inconnue"
L["Unknown source. Please report it. Source="] 					= "Source inconnue. Veuillez le rapporter. Source = "
L["Unknown NPC. Please report it. Source="] 					= "PNJ inconnu. Veuillez le rapporter. Source = "
L["Could not load the UI. Reason: "] 							= "Impossible de charger l'UI. Raison : "
L["Search"] 													= "Recherche"
L["Seasonal"] 													= "Saisonnier"
L["Intermittent"] 												= "Intermittant"
L["Roving"] 													= "Itinérant"
-- Vendors
L["Abigail Shiel"] 												= "Abigail Shiel"
L["Aendel Windspear"] 											= "Aendel Lancevent"
L["Alchemist Pestlezugg"] 										= "Alchimiste Pilonzugg"
L["Alexandra Bolero"] 											= "Alexandra Bolero"
L["Algernon"] 													= "Algernon"
L["Amy Davenport"] 												= "Amy Davenport"
L["Andrew Hilbert"] 											= "Andrew Hilbert"
L["Androd Fadran"] 												= "Androd Fadran"
L["Argent Quartermaster Hasana"] 								= "Intendante de l'Aube d'argent Hasana"
L["Argent Quartermaster Lightspark"] 							= "Intendant de l'Aube d'argent Feuzopoudre"
L["Balai Lok'Wein"] 											= "Balai Lok'Wein"
L["Bale"] 														= "Bale"
L["Banalash"] 													= "Banalash"
L["Blimo Gadgetspring"] 										= "Blimo Gadgettaressort"
L["Blixrez Goodstitch"] 										= "Blixrez Beaupiqué"
L["Blizrik Buckshot"] 											= "Blizrik Chevrotine"
L["Bliztik"] 													= "Bliztik"
L["Bombus Finespindle"] 										= "Bombus Finfuseau"
L["Borya"] 														= "Borya"
L["Brienna Starglow"] 											= "Brienna Luisétoile"
L["Bro'kin"] 													= "Fra'kas"
L["Bronk"] 														= "Bronk"
L["Catherine Leland"] 											= "Catherine Leland"
L["Christoph Jeffcoat"] 										= "Christoph Jeffcoat"
L["Clyde Ranthal"] 												= "Clyde Ranthal"
L["Constance Brisboise"] 										= "Constance Brisboise"
L["Corporal Bluth"] 											= "Caporal Bluth"
L["Cowardly Crosby"] 											= "Crosby la Trouille"
L["Crazk Sparks"] 												= "Crazk Escarbilles"
L["Dalria"] 													= "Dalria"
L["Daniel Bartlett"] 											= "Daniel Bartlett"
L["Danielle Zipstitch"] 										= "Danielle Vivaiguille"
L["Darian Singh"] 												= "Darian Singh"
L["Darnall"] 													= "Darnall"
L["Defias Profiteer"] 											= "Profiteur défias"
L["Deneb Walker"] 												= "Deneb Walker"
L["Derak Nightfall"] 											= "Derak Crépuscule"
L["Dirge Quikcleave"] 											= "Dirge Hachillico"
L["Drac Roughcut"] 												= "Drac Taillebrute"
L["Drake Lindgren"] 											= "Drake Lindgren"
L["Drovnar Strongbrew"] 										= "Drovnar Fortebière"
L["Elynna"] 													= "Elynna"
L["Emrul Riknussun"] 											= "Emrul Riknussun"
L["Erika Tate"] 												= "Erika Tate"
L["Evie Whirlbrew"] 											= "Evie Tourbouillon"
L["Fradd Swiftgear"] 											= "Fradd Vifembraye"
L["Fyldan"] 													= "Fyldan"
L["Gagsprocket"] 												= "Muserouage"
L["Gearcutter Cogspinner"] 										= "Coupebraquet Tournepignon"
L["George Candarte"] 											= "George Candarte"
L["Gharash"] 													= "Gharash"
L["Ghok'kah"] 													= "Ghok'kah"
L["Gigget Zipcoil"] 											= "Giguette Filebobine"
L["Gikkix"] 													= "Gikkix"
L["Gina MacGregor"] 											= "Gina MacGregor"
L["Gloria Femmel"] 												= "Gloria Femmel"
L["Glyx Brewright"] 											= "Glyx Bonbouilleur "
L["Gnaz Blunderflame"] 											= "Gnaz Bourdeflamme"
L["Gretta Ganter"] 												= "Gretta Ganter"
L["Grimtak"] 													= "Grimtak"
L["Hagrus"] 													= "Hagrus"
L["Hammon Karwn"] 												= "Hammon Karwn"
L["Harggan"] 													= "Harggan"
L["Harklan Moongrove"] 											= "Harklan Bosquelune"
L["Harlown Darkweave"] 											= "Harlown Tisselombre"
L["Harn Longcast"] 												= "Harn Jette-loin"
L["Heldan Galesong"] 											= "Heldan Chantetrombe"
L["Helenia Olden"] 												= "Helenia Olden"
L["Himmik"] 													= "Himmik"
L["Hula'mahi"] 													= "Hula'mahi"
L["Jabbey"] 													= "Jabbey"
L["Jandia"] 													= "Jandia"
L["Janet Hommers"] 												= "Janet Hommers"
L["Jangdor Swiftstrider"] 										= "Jangdor Rôdeur-agile"
L["Jannos Ironwill"] 											= "Jannos Volonté-de-fer"
L["Jaquilina Dramet"] 											= "Jaquilina Dramet"
L["Jase Farlane"] 												= "Jase Farlane"
L["Jazzrik"] 													= "Jazzrik"
L["Jeeda"] 														= "Jeeda"
L["Jennabink Powerseam"] 										= "Jennabink Doublepiqûre"
L["Jessara Cordell"] 											= "Jessara Cordell"
L["Jinky Twizzlefixxit"] 										= "Jinky Rafistépingle"
L["Joseph Moore"] 												= "Joseph Moore"
L["Jubie Gadgetspring"] 										= "Jubie Gadgettaressort"
L["Jun'ha"] 													= "Jun'ha"
L["Jutak"] 														= "Jutak"
L["Kaita Deepforge"] 											= "Kaita Forge-profonde"
L["Kalldan Felmoon"] 											= "Kalldan Gangrelune"
L["Kania"] 														= "Kania"
L["Keena"] 														= "Keena"
L["Kelsey Yance"] 												= "Kelsey Yance"
L["Kendor Kabonka"] 											= "Kendor Kabonka"
L["Khara Deepwater"] 											= "Khara Eau-profonde"
L["Khole Jinglepocket"] 										= "Samatiraine Tintinnabourse"
L["Kiknikle"] 													= "Piéniklé"
L["Killian Sanatha"] 											= "Killian Sanatha"
L["Kilxx"] 														= "Kilxx"
L["Kireena"] 													= "Kireena"
L["Kithas"] 													= "Kithas"
L["Knaz Blunderflame"] 											= "Knaz Bourdeflamme"
L["Kor'geld"] 													= "Kor'geld"
L["Kriggon Talsone"] 											= "Kriggon Talsone"
L["Krinkle Goodsteel"] 											= "Froisse Bonnacier"
L["Kulwia"] 													= "Kulwia"
L["Kzixx"] 														= "Kzixx"
L["Laird"] 														= "Laird"
L["Lardan"] 													= "Lardan"
L["Leo Sarn"] 													= "Leo Sarn"
L["Leonard Porter"] 											= "Leonard Porter"
L["Lieutenant General Andorov"] 								= "Général de division Andorov"
L["Lilly"] 														= "Lilly"
L["Lindea Rabonne"] 											= "Lindea Rabonne"
L["Lizbeth Cromwell"] 											= "Lizbeth Cromwell"
L["Logannas"] 													= "Logannas"
L["Lokhtos Darkbargainer"] 										= "Lokhtos Sombrescompte"
L["Lorelae Wintersong"] 										= "Lorelae Chante-hiver"
L["Magnus Frostwake"] 											= "Magnus Givréveil"
L["Mahu"] 														= "Mahu"
L["Mallen Swain"] 												= "Mallen Legallant"
L["Malygen"] 													= "Malygen"
L["Maria Lumere"] 												= "Maria Lumere"
L["Martine Tramblay"] 											= "Martine Tramblay"
L["Masat T'andr"] 												= "Masat T'andr"
L["Master Craftsman Omarion"] 									= "Maître-artisan Omarion"
L["Mavralyn"] 													= "Mavralyn"
L["Mazk Snipeshot"] 											= "Mazk Tiraubut"
L["Meilosh"] 													= "Meilosh"
L["Micha Yance"] 												= "Micha Yance"
L["Millie Gregorian"] 											= "Millie Gregorian"
L["Mishta"] 													= "Mishta"
L["Montarr"] 													= "Montarr"
L["Muuran"] 													= "Muuran"
L["Mythrin'dir"] 												= "Mythrin'dir"
L["Naal Mistrunner"] 											= "Naal Cours-la-brume"
L["Namdo Bizzfizzle"] 											= "Namdo Ventaperte"
L["Nandar Branson"] 											= "Nandar Branson"
L["Nardstrum Copperpinch"] 										= "Prijunic Chipsou"
L["Narj Deepslice"] 											= "Narj Tranchefond"
L["Narkk"] 														= "Narkk"
L["Nata Dawnstrider"] 											= "Nata Aube-glorieuse"
L["Nergal"] 													= "Nergal"
L["Nerrist"] 													= "Nerrist"
L["Nessa Shadowsong"] 											= "Nessa Chantelombre"
L["Nina Lightbrew"] 											= "Nina Bouillelume"
L["Nioma"] 														= "Nioma"
L["Nyoma"] 														= "Nyoma"
L["Ogg'marr"] 													= "Ogg'marr"
L["Old Man Heming"] 											= "Vieil Heming"
L["Otho Moji'ko"] 												= "Otho Moji'ko"
L["Outfitter Eric"] 											= "Eric le tailleur"
L["Penney Copperpinch"] 										= "Praintan Chipsou"
L["Plugger Spazzring"] 											= "Lanfiche Brouillecircuit"
L["Pratt McGrubben"] 											= "Pratt McGrubben"
L["Qia"] 														= "Qia"
L["Quartermaster Miranda Breechlock"] 							= "Intendante Miranda Coinceculasse"
L["Ranik"] 														= "Ranik"
L["Rann Flamespinner"] 											= "Rann Tisseflamme"
L["Rartar"] 													= "Rartar"
L["Rikqiz"] 													= "Rikqiz"
L["Rin'wosho the Trader"] 										= "Rin'wosho le marchand"
L["Rizz Loosebolt"] 											= "Rizz Court-boulon"
L["Ronald Burch"] 												= "Ronald Burch"
L["Ruppo Zipcoil"] 												= "Ruppo Filebobine"
L["Saenorion"] 													= "Saenorion"
L["Seersa Copperpinch"] 										= "Sokyriel Chipsou"
L["Sewa Mistrunner"] 											= "Sewa Cours-la-brume"
L["Shandrina"] 													= "Shandrina"
L["Shankys"] 													= "Shankys"
L["Sheendra Tallgrass"] 										= "Sheendra Hautes-herbes"
L["Shen'dralar Provisioner"] 									= "Approvisionneur Shen'dralar"
L["Sheri Zipstitch"] 											= "Sheri Vivaiguille"
L["Smudge Thunderwood"] 										= "Smudge Tonnebois"
L["Soolie Berryfizz"] 											= "Soolie Baiedesbulles"
L["Sovik"] 														= "Sovik"
L["Stuart Fleming"] 											= "Stuart Fleming"
L["Sumi"] 														= "Sumi"
L["Super-Seller 680"] 											= "Super-Vendeur 680"
L["Tamar"] 														= "Tamar"
L["Tansy Puddlefizz"] 											= "Tansy Flaquabulle"
L["Tarban Hearthgrain"] 										= "Tarban Blé-du-foyer"
L["Tari'qa"] 													= "Tari'qa"
L["Thaddeus Webb"] 												= "Thaddeus Webb"
L["Tharynn Bouden"] 											= "Tharynn Bouden"
L["Tilli Thistlefuzz"] 											= "Tilli Duvechardon"
L["Truk Wildbeard"] 											= "Truk Barbe-hirsute"
L["Tunkk"] 														= "Tunkk"
L["Ulthaan"] 													= "Ulthaan"
L["Ulthir"] 													= "Ulthir"
L["Uthok"] 														= "Uthok"
L["Vaean"] 														= "Vaean"
L["Valdaron"] 													= "Valdaron"
L["Vargus"] 													= "Vargus"
L["Veenix"] 													= "Veenix"
L["Vendor-Tron 1000"] 											= "Vendeur-o-tron 1000"
L["Vharr"] 														= "Vharr"
L["Vivianna"] 													= "Vivianna"
L["Vizzklick"] 													= "Vizzklick"
L["Wenna Silkbeard"] 											= "Wenna Barbe-de-soie"
L["Werg Thickblade"] 											= "Werg Largelame"
L["Wik'Tar"] 													= "Wik'Tar"
L["Worb Strongstitch"] 											= "Worb Point-solide"
L["Wrahk"] 														= "Wrahk"
L["Wulan"] 														= "Wulan"
L["Wulmort Jinglepocket"] 										= "Karfour Tintinnabourse"
L["Wunna Darkmane"] 											= "Wunna Sombre-crin"
L["Xandar Goodbeard"] 											= "Xandar Bonnebarbe"
L["Xen'to"] 													= "Xen'to"
L["Xizk Goodstitch"] 											= "Xizk Beaupiqué"
L["Xizzer Fizzbolt"] 											= "Xizzer Klaqueboulon"
L["Yonada"] 													= "Yonada"
L["Yuka Screwspigot"] 											= "Yuka Fermevanne"
L["Zan Shivsproket"] 											= "Zan Tympeustache"
L["Zannok Hidepiercer"] 										= "Zannok Percecuir"
L["Zansoa"] 													= "Zansoa"
L["Zarena Cromwind"] 											= "Zarena Cromwind"
L["Zargh"] 														= "Zargh"
L["Zixil"] 														= "Zixil"
L["Zorbin Fandazzle"] 											= "Zorbin Poudrozieu"
L["Crimson Battle Mage"] 										= "Mage de bataille cramoisi"
L["Spectral Researcher"] 										= "Chercheur spectral"
L["Scholomance Dark Summoner"] 									= "Invocateur des ténèbres de la Scholomance"
L["Blackrock Battlemaster"] 									= "Maître de guerre Rochenoire"
L["Ghoul Ravener"] 												= "Goule vorace"
-- Burning Crusade
L["Deynna"] 													= "Deynna"
L["Eiin"] 														= "Eiin"
L["Neii"] 														= "Neii"
L["Borto"] 														= "Borto"
L["Mathar G'ochar"] 											= "Mathar G'ochar"
L["Andrion Darkspinner"] 										= "Andrion Sombrefileur"
L["Arrond"] 													= "Arrond"
L["Nasmara Moonsong"] 											= "Nasmara Lunechant"
L["Gidge Spellweaver"] 											= "Gidge Tisse-sort"
L["Karaaz"] 													= "Karaaz"
L["Nakodu"] 													= "Nakodu"
L["Quartermaster Endarin"] 										= "Intendant Endarin"
L["Quartermaster Enuril"] 										= "Intendant Enuril"
L["Fedryen Swiftspear"] 										= "Fedryen Vivelance"
L["Muheru the Weaver"] 											= "Muheru le Tisserand"
L["Zurai"] 														= "Zurai"
L["Koren"] 														= "Koren"
L["Krek Cragcrush"] 											= "Krek Cassecombe"
L["Loolruna"] 													= "Loolruna"
L["Aaron Hollman"] 												= "Aaron Hollman"
L["Arras"] 														= "Arras"
L["Eriden"] 													= "Eriden"
L["Mari Stonehand"] 											= "Mari Main-de-pierre"
L["Rohok"] 														= "Rohok"
L["Quartermaster Urgronn"] 										= "Intendant Urgronn"
L["Logistics Officer Ulrike"] 									= "Officier de logistique Ulrike"
L["Haalrun"] 													= "Haalrun"
L["Seer Janidi"] 												= "Prophète Janidi"
L["Daga Ramba"] 												= "Daga Ramba"
L["Alurmi"] 													= "Alurmi"
L["Almaador"] 													= "Almaador"
L["Mycah"] 														= "Mycah"
L["Trader Narasu"] 												= "Marchand Nasaru"
L["Altaa"] 														= "Altaa"
L["Melaris"] 													= "Melaris"
L["Skreah"] 													= "Skreah"
L["Alchemist Gribble"] 											= "Alchimiste Limnorie"
L["Apothecary Antonivich"] 										= "Apothicaire Antonivitch"
L["Leeli Longhaggle"] 											= "Leeli Longnégoce"
L["Burko"] 														= "Burko"
L["Aresella"] 													= "Aresella"
L["Baxter"] 													= "Baxter"
L["Gaston"] 													= "Gaston"
L["Naka"] 														= "Naka"
L["Landraelanis"] 												= "Landraelanis"
L["Master Chef Mouldier"] 										= "Maître cuisinier Mouldier"
L["Innkeeper Grilka"] 											= "Aubergiste Grilka"
L["Supply Officer Mills"] 										= "Officier de ravitaillement Mills"
L["Doba"] 														= "Doba"
L["Gambarinka"] 												= "Gambarinka"
L["Fazu"] 														= "Fazu"
L["Innkeeper Biribi"] 											= "Aubergiste Biribi"
L["Rungor"] 													= "Rungor"
L["Nula the Butcher"] 											= "Nula la Bouchère"
L["Uriku"] 														= "Uriku"
L["Cookie One-Eye"] 											= "Qu'un-œil le cuistot"
L["Sid Limbardi"] 												= "Sid Limbardi"
L["Arred"] 														= "Arred"
L["Gelanthis"] 													= "Gelanthis"
L["Neal Allen"] 												= "Neal Allen"
L["Felika"] 													= "Felika"
L["Edna Mullby"] 												= "Edna Mullby"
L["Burbik Gearspanner"] 										= "Burbik Clépharnaüm"
L["Felicia Doan"] 												= "Felicia Doan"
L["Shadi Mistrunner"] 											= "Shadi Cours-la-brume"
L["Apprentice Darius"] 											= "Apprenti Darius"
L["Kalaen"] 													= "Kalaen"
L["Tatiana"] 													= "Tatiana"
L["Feera"] 														= "Feera"
L["Viggz Shinesparked"] 										= "Viggz Fibriller"
L["Yatheon"] 													= "Yatheon"
L["Daggle Ironshaper"] 											= "Détrempe Sculptefer"
L["Mixie Farshot"] 												= "Mixie Vise-loin"
L["Wind Trader Lathrai"] 										= "Marchand des vents Lathrai"
L["Lebowski"] 													= "Lebowski"
L["Captured Gnome"] 											= "Gnome capturé"
L["Provisioner Nasela"] 										= "Approvisionneur Nasela"
L["Haferet"] 													= "Haferet"
L["Zaralda"] 													= "Zaralda"
L["Cro Threadstrong"] 											= "Cro Fermefil"
L["Thomas Yance"] 												= "Thomas Yance"
L["Juno Dufrain"] 												= "Juno Dufrain"
L["Vodesiin"] 													= "Vodesiin"
L["Andormu"] 													= "Andormu"
L["Aged Dalaran Wizard"] 										= "Sorcier de Dalaran âgé"
L["Madame Ruby"] 												= "Madame Ruby"
L["Egomis"] 													= "Egomis"
L["Lyna"] 														= "Lyna"
L["Okuno"] 														= "Okuno"
L["Indormi"] 													= "Indormi"
L["Eldara Dawnrunner"] 											= "Eldara Coursaurore"
L["Shaani"] 													= "Shaani"
L["Arille Azuregaze"] 											= "Arille Mirazur"
L["Derek Odds"] 												= "Derek Chance"
L["Misensi"] 													= "Misensi"
L["Haughty Modiste"] 											= "Hautaine Lamodiste"
-- TODO
L["Quartermaster Davian Vaclav"] = "Quartermaster Davian Vaclav"
L["Quartermaster Jaffrey Noreliqe"] = "Quartermaster Jaffrey Noreliqe"
L["Paulsta'ats"] = "Paulsta'ats"
L["Vanessa Sellers"] = "Vanessa Sellers"
L["Gara Skullcrush"] = "Gara Skullcrush"
L["Sebastian Crane"] = "Sebastian Crane"
L["Logistics Officer Brighton"] = "Logistics Officer Brighton"
L["Logistics Officer Silverstone"] = "Logistics Officer Silverstone"
L["Ontuvo"] = "Ontuvo"
L["Tanak"] = "Tanak"
L["Duchess Mynx"] = "Duchess Mynx"
L["Geen"] = "Geen"
L["Sairuk"] = "Sairuk"
L["Tanaika"] = "Tanaika"
L["Tiffany Cartier"] = "Tiffany Cartier"
L["Archmage Alvareaux"] = "Archmage Alvareaux"
L["Lillehoff"] = "Lillehoff"
L["Cielstrasza"] = "Cielstrasza"
L["Veteran Crusader Aliocha Segard"] = "Veteran Crusader Aliocha Segard"
L["Knight Dameron"] = "Knight Dameron"
L["Morgan Day"] = "Morgan Day"
L["Stone Guard Mukar"] = "Stone Guard Mukar"
L["Braeg Stoutbeard"] = "Braeg Stoutbeard"
L["Bryan Landers"] = "Bryan Landers"
L["Captain O'Neal"] = "Captain O'Neal"
L["Lady Palanseer"] = "Lady Palanseer"
L["Combine profit from auctioneer advanced"] = "Combine profit from auctioneer advanced"
L["Show cost of combine and profit from auctioneer advanced"] = "Show cost of combine and profit from auctioneer advanced"
L["Market Value"] = "Market Value"
L["Auctioneer Market Value Profit"] = "Auctioneer Market Value Profit"
L["Show the profit calculation from Auctioneer Market Value in tooltip"] = "Show the profit calculation from Auctioneer Market Value in tooltip"
